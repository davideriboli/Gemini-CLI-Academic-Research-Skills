...
Guidelines for coordinating multi-person academic research teams using the pipeline. Gemini CLI runs as a single-user session; this protocol documents the **human coordination layer** that wraps around pipeline executions.
...
Person A (Research Lead + Lead Author):
  - Runs Stage 1 (RESEARCH) with their Gemini session
  - Runs Stage 2 (WRITE) with their Gemini session
...
- Gemini CLI runs as a single-user session; this protocol documents the HUMAN coordination layer
- Each team member runs their own pipeline stages independently in separate Gemini sessions
